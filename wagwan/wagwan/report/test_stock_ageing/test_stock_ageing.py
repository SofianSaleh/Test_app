...
from frappe.utils.data import cint, date_diff, flt
...
def get_columns():
    return [
        ...
        {
            "label": _("31-60 Amount"),
            "fieldname": "31_60_amount",
            "fieldtype": "Data",
            "width": 150,
        },
        {
        ...
        {
            "label": _("61-90 Amount"),
            "fieldname": "61_90_amount",
            "fieldtype": "Data",
            "width": 150,
        },
        {
            "label": _("Over 90 QTY"),
            "fieldname": "over_90_qty",
            "fieldtype": "Data",
            "width": 150,
        },
        {
            "label": _("Over 90 Amount"),
            "fieldname": "over_90_amount",
            "fieldtype": "Data",
            "width": 150,
        },
        {
...
def get_filters(filters):
    ...
    if filters.get("company"):
        fil["company"] = filters.get("company")
    if filters.get("to_date"):

        fil["to_date"] = filters.get("to_date")
    if filters.get("item_code"):
        fil["item_code"] = filters.get("item_code")
    return fil


def get_data(filters):
    fil = get_filters(filters)

    item_details = FIFOSlots(fil).generate()
    data = format_report_data(fil, item_details, fil.get("to_date"))
...
def format_report_data(
    ...
    for _item, item_dict in item_details.items():
        ...
        (
            ...
            above_range3,
            above_range3_amount,
        ) = get_range_age(filters, fifo_queue, to_date, item_dict)
...
def get_range_age(
    filters: Filters, fifo_queue: list, to_date: str, item_dict: dict
) -> tuple:
    precision = cint(
        frappe.db.get_single_value("System Settings", "float_precision", cache=True)
    )

    range1 = range1_amount = range2 = range2_amount = range3 = range3_amount = (
        above_range3
    ) = above_range3_amount = 0.0
    ...
    for item in fifo_queue:
        ...
        valuation_rate = frappe.db.get_value(
            "Bin",
            {
                "item_code": item_details.get("name"),
                "warehouse": item_details.get("warehouse"),
            },
            "valuation_rate",
            as_dict=True,
        )
        ...
        if age <= flt(filters["range1"]):
            ...
            range1_amount = qty * valuation_rate.get("valuation_rate")

        elif age <= flt(filters["range2"]):
            range2 = flt(range2 + qty, precision)
            range2_amount = qty * valuation_rate.get("valuation_rate")

        elif age <= flt(filters["range3"]):
            range3 = flt(range3 + qty, precision)
            range3_amount = qty * valuation_rate.get("valuation_rate")

        else:
            above_range3 = flt(above_range3 + qty, precision)
            above_range3_amount = qty * valuation_rate.get("valuation_rate")
